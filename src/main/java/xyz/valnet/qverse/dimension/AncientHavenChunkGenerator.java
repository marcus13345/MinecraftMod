...
public class AncientHavenChunkGenerator extends NoiseChunkGenerator<AncientHavenGenerationSettings> {

	private static final float[] field_222576_h = Util.make(new float[25], (p_222575_0_) -> {
		for(int i = -2; i <= 2; ++i) {
			for(int j = -2; j <= 2; ++j) {
				final float f = 10.0F / MathHelper.sqrt((float) (i * i + j * j) + 0.2F);
	...
	public AncientHavenChunkGenerator(final IWorld worldIn, final BiomeProvider biomeProviderIn,
			final AncientHavenGenerationSettings generationSettingsIn) {
		super(worldIn, biomeProviderIn, 4, 8, 256, generationSettingsIn, true);
	...
	@Override
	protected void fillNoiseColumn(final double[] noiseColumn, final int noiseX, final int noiseZ) {
		final double d0 = (double) 684.412F;
		final double d1 = (double) 684.412F;
		final double d2 = 8.555149841308594D;
		final double d3 = 4.277574920654297D;
		final int i = -10;
		final int j = 3;
   ...
   private double getNoiseDepthAt(int noiseX, int noiseZ) {
	...
	public List<Biome.SpawnListEntry> getPossibleCreatures(final EntityClassification creatureType, final BlockPos pos) {
	...
	public void spawnMobs(final ServerWorld worldIn, final boolean spawnHostileMobs, final boolean spawnPeacefulMobs) {
